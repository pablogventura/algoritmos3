...
"""Orquestador del algoritmo Edmonds-Karp (inyección de dependencias)."""
...
class EstadoEK:
    """
    Orquestador del algoritmo Edmonds-Karp.
    Depende de una red (MatrizCap) y un manejador de salida (Strategy).
    """
    ...
    def leer_un_lado(self):
        ...
        if red is None:
            return 0
        # Copiar aristas leídas a self._cap (solo se leyó una línea en el diseño original)
        # El contrato original era leer una línea por llamada; read_network lee todo.
        # Mantenemos compatibilidad: leer con input() una sola línea.
        try:
            entrada = input("")
        except EOFError:
            return 0
        m = re.match(r"^(\d+)\s+(\d+)\s+(\d+)\s*$", entrada)
        if m:
    ...
    def aumentar_flujo(self):
        ...
        self._output.on_bfs_and_path(
            self._cant_caminos, str(bfs), str(camino)
        )
        return 1
    ...
    def run(self, stream=None):
        ...
        Devuelve el valor del flujo maximal, o -1 si hubo error al leer.
        """
