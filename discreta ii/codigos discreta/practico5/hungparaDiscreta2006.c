...
int col_que_matchea_la_fila[TAM];	//indica que columna matchea la fila. sera negativo si no esta matcheada
int fila_que_matchea_la_col[TAM];	//idem pero al reves: indica con que fila estamos matcheando la columna.
int etiqueta_de_columna[TAM];	/*para etiquetear columnas con nombres de filas para poder luego extender el matching
...
int S[TAM];			/* Este sera el ``S'' de clase, i.e..,  donde se guardan al principio las filas que no matcheamos
...
int Minimo;			//para calcular el minimo de Sxcomplemento(Gamma(S)), y tambien para  calcular min filas o columnas.
int restar_de_fila[TAM];	/*esto es lo que debemos restarle a las filas  de S cuando cambiemos la matriz.
				   para ahorrar tiempo, en vez de ir y restarlo, se lo guarda en un registro. */
int sumar_a_col[TAM];		/*aca guardamos lo que le sumariamos a las columnas de Gamma(S).
				   Al igual que con las filas, no lo sumamos realmente. */
int xy_real;			//cuando deba ver la entrada ``real'' xy, la guardare aqui mientras la examino.
...
int extension;			//cuando extendamos el match, una flag para saber cuando terminamos.
int verboso;			//habra tres niveles de verbosidad, segun lo que quiera que me muetre.
...
{
  while (i < s && frigidez)	//solo estoy aca si s no es cero, asi que el loop comienza bien.
    {
...
}				// End extender matching (cuando salimos, extendimos el match, i.e.., termina un paso)
...
int
main (int argc, char *argv[])	//<------------------------ACA ESTA EL MAIN----------------------------
{
  if (argc != 2)
    {
      printf
